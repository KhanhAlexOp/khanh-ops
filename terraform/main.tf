...
resource "aws_security_group" "allow_web_ssh" {
  ...
  ingress {
    ...
    to_port     = 80
    protocol    = "tcp"
    cidr_blocks = ["0.0.0.0/0"]  # Allow HTTP for testing
  }
...
resource "aws_instance" "dev_server" {
  ami                    = var.aws_ami # Ubuntu AMI (check your region)
  instance_type          = "t2.micro"
  key_name               = var.aws_ssh_key         # Replace with your AWS key pair name
  security_groups        = [aws_security_group.allow_web_ssh.name]
  user_data              = file("install_docker.sh")  # Run startup script
  tags = {
    Name = "KhanhDevOps"
  }
  ...
  provisioner "file" {
    ...
    connection {
      ...
      private_key = file("./../aws_config/khanh-devops.pem")  # Path to your local PEM file
      host        = self.public_ip
    }
  }
...
provisioner "file" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo docker-compose -f /home/ubuntu/docker-compose.yml up -d"  # Use sudo to run docker-compose
    ]
